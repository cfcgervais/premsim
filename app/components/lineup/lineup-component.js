angular.module('sim')
    ...
    .controller('lineupController', function($scope, $http, $state, Service){
        let dumpUrl = "https://fantasy.premierleague.com/drf/bootstrap-static";
        ...
        $http.get(dumpUrl).success(function(data){
            ...
            function Roster(){
                ...
                for(var i = 0; i < teamArr.length; i++){
                    if(teamArr[i].name == mySquad){
                        players = teamArr[i].players
                    }
                }
                ...
                for(i = 0; i < players.length; i++){
                    ...
                    template += `<span id="${player.web_name}"><button class="btn-danger remove-btn" id="${player.web_name}">Remove</button><button class="btn-success start-btn" id="${player.web_name}">Start</button> ${player.web_name}<br></span>`
                    }
            ...
            Service.myEleven = myEleven;
            Service.jsonData = data;

            // if(myEleven.length > 10){
            //     done = true;
            // }
